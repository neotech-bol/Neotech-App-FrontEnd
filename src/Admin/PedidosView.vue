          ...
          <div v-for="(item, index) in filteredOrders" :key="item.id" class="card mb-3 mx-3">
            <div class="card-body">
              ...
              <p class="card-text">
                ...
                <strong>Productos:</strong> <span class="badge bg-info rounded-pill">{{ item.productos?.length || 0
                  }}</span><br>
                      ...
                      <div class="list-group-item" v-for="(item, index) in detailOrder.productos" :key="item.id">
                        <div class="d-flex justify-content-between align-items-start">
                          <div>
                            ...
                            <div class="text-muted small mt-1">
                              Color: {{ item.pivot?.color ? item.pivot.color : 'N/A' }} <!-- Muestra el color -->
                            </div>
                          </div>
                  ...
                  <div class="card-body">
                    <ul class="list-group list-group-flush">
                      ...
                      <li class="list-group-item d-flex justify-content-between px-0" v-if="detailOrder.cupon">
                        ...
                        <span class="badge bg-info">{{ detailOrder.cupon?.codigo }} ({{ detailOrder.cupon?.descuento
                          }}%)</span>
                      </li>
                      <li class="list-group-item d-flex justify-content-between px-0">
...
import { completeOrder, generaPDFPedidoID, generateExcel, indexPedidos, modelsAll, showPedido } from '@/Services/PedidoService';
...
const models = ref([]); // Suponiendo que tienes un array de modelos

const getModelName = (modeloId) => {
  const model = models.value.find(m => m.id === modeloId);
  return model ? model.nombre : 'Desconocido'; // Devuelve el nombre del modelo o 'Desconocido' si no se encuentra
};
// Computed
...
const fetchOrdersPDFId = async (id) => {
  try {
    ...
    if (contentType === 'application/json') {
      // If it's JSON, we're probably getting a base64 or URL response
      ...
      // If it's a PDF, handle it as before
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `pedido_${id}.pdf`);
...
const loadModels = async () => {
  try {
    const { data } = await modelsAll(); // Asegúrate de tener una función para obtener los modelos
    models.value = data;
